...
import { pickColorByBins, getActivePalette } from '../palette.js'

let isZh = true
let dateFormat = new Intl.DateTimeFormat(isZh ? 'zh-CN' : 'en-US', { month: isZh ? 'numeric' : 'short', day: 'numeric', timeZone: 'UTC' })
...
const getSettings = () => {
  return new Promise((resolve) => {
    ...
    if (typeof chrome !== 'undefined' && chrome.storage && chrome.storage.local) {
      chrome.storage.local.get(['toggleSetting', 'language', 'language_github', 'paletteName', 'paletteName_github', 'customBinsPalette'], (settings) => {
        toggleSetting = settings.toggleSetting ?? 'cubes'
        paletteName = settings.paletteName ?? settings.paletteName_github ?? 'github'
        customPalette = Array.isArray(settings.customBinsPalette) ? settings.customBinsPalette.map((x)=>String(x).replace('#','')) : null
        const langRaw = settings.language ?? settings.language_github
        if (langRaw) {
      ...
      paletteName = localStorage.paletteName ?? localStorage.paletteName_github ?? 'github'
      const langLocal = localStorage.language ?? localStorage.language_github
      if (langLocal) {
...
const loadStats = () => {
  ...
  dateBest ||= (isZh ? '未发现活动' : 'No activity found')
...
const renderStats = () => {
  ...
  const labels = {
    contributions: isZh ? '贡献' : 'Contributions',
    total: isZh ? '总计' : 'Total',
    week: isZh ? '本周' : 'This week',
    bestDay: isZh ? '最佳日' : 'Best day',
    average: isZh ? '平均' : 'Average',
    perDay: isZh ? '/ 天' : '/ day',
    streaks: isZh ? '连续贡献' : 'Streaks',
    longest: isZh ? '最长' : 'Longest',
    current: isZh ? '当前' : 'Current',
    days: isZh ? '天' : 'days',
    noCurrent: isZh ? '无当前连续' : 'No current streak',
    noLongest: isZh ? '无最长连续' : 'No longest streak',
    noActivity: isZh ? '未发现活动' : 'No activity found'
  }

  let topMarkup = `
    <div class="position-absolute top-0 right-0 mt-3 mr-5">
      <h5 class="mb-1">${labels.contributions}</h5>
      <div class="d-flex flex-justify-between rounded-2 border px-1 px-md-2">
        <div class="p-2">
          ...
          <span class="d-block text-small text-bold">${labels.total}</span>
  ...
  if (!viewingYear) {
    topMarkup += `
      <div class="p-2 d-none d-xl-block">
        ...
        <span class="d-block text-small text-bold">${labels.week}</span>
  ...
  topMarkup += `
      <div class="p-2">
        ...
        <span class="d-block text-small text-bold">${labels.bestDay}</span>
        <span class="d-none d-sm-block text-small color-fg-muted">${dateBest}</span>
      </div>
    </div>
    <p class="mt-1 text-right text-small">
      ${labels.average}: <span class="text-bold color-fg-success">${averageCount}</span> <span class="color-fg-muted">${labels.perDay}</span>
      </p>
    </div>
  `
  ...
  let bottomMarkup = `
    <div class="position-absolute bottom-0 left-0 ml-5 mb-6">
      <h5 class="mb-1">${labels.streaks}</h5>
      <div class="d-flex flex-justify-between rounded-2 border px-1 px-md-2">
        <div class="p-2">
          <span class="d-block f2 text-bold color-fg-success lh-condensed">${streakLongest} <span class="f4">${labels.days}</span></span>
          <span class="d-block text-small text-bold">${labels.longest}</span>
  ...
  if (!viewingYear) {
    bottomMarkup += `
          <div class="p-2">
            <span class="d-block f2 text-bold color-fg-success lh-condensed">${streakCurrent} <span class="f4">${labels.days}</span></span>
            <span class="d-block text-small text-bold">${labels.current}</span>
  ...
  const observedContainer = document.querySelector('html')
    ...
    if (typeof chrome !== 'undefined' && chrome.storage && chrome.storage.onChanged) {
      chrome.storage.onChanged.addListener((changes, areaName) => {
        ...
        if (changes.customBinsPalette) {
          const next = changes.customBinsPalette.newValue
          if (Array.isArray(next) && next.length === 4) {
            customPalette = next.map((x)=>String(x).replace('#',''))
            paletteName = 'custom'
            applyPaletteAndRerender()
          }
        } else if (changes.paletteName) {
          const pv = String(changes.paletteName.newValue || '').toLowerCase()
          if (pv) {
            paletteName = pv
            applyPaletteAndRerender()
          }
